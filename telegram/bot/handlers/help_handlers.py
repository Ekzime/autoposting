...
async def cmd_help(message: Message):
    ...
    if not AuthService.verify_session(user_id):
        # Краткая справка для неавторизованных
        help_text = """
🤖 <b>AIPosting Bot</b> - система управления контентом

<b>📋 Доступные команды:</b>
/start - начать работу с ботом
/login - войти в систему (требуется пароль)
/help - показать эту справку

🔐 <b>Для доступа к полному функционалу необходима авторизация.</b>
Используйте команду /login для входа в систему.

📧 Если у вас нет доступа, обратитесь к администратору.
"""
        await message.answer(help_text, parse_mode="HTML")
        logger.info(f"Показана краткая справка неавторизованному пользователю {user_id}")
        return
    
    # Полная справка для авторизованных админов
    commands_text = """
...
/session_status - проверить статус сессии
...
"""
    ...
    logger.info(f"Показан полный список команд пользователю {user_id}")
